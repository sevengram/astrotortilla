...
class Separation(object):
    ...
    def __init__(self, coord1, coord2):
        ...
        if type(coord1) == float:
            ...
            C = sin(radians(dDec) / 2) ** 2 + cos(radians(coord1.dec)) * cos(radians(coord2.dec)) * sin(
                radians(dRA) / 2) ** 2
            self.__sep = degrees(2 * asin(sqrt(abs(C))))
            # initial bearing
            y = sin(radians(dRA)) * cos(radians(coord2.dec))
            x = cos(radians(coord1.dec)) * sin(radians(coord2.dec)) - sin(radians(coord1.dec)) * cos(
                radians(coord2.dec)) * cos(radians(dRA))
    ...
    def __add__(self, other):
        ...
        destDec = asin(sin(radians(other.dec)) * cos(radians(self.__sep)) + cos(radians(other.dec)) * sin(
            radians(self.__sep)) * cos(radians(self.__bear)))

        destRA = radians(other.RA) + atan2(
            sin(radians(self.__bear)) * sin(radians(self.__sep)) * cos(radians(other.dec)),
            cos(radians(self.__sep)) - sin(radians(other.dec)) * sin(destDec))
...
def alignment(angle1, angle2):
    ...
    # normalize to 0..360
    if angle1 < 0:
        angle1 += (1 + int(-angle1) / 360) * 360
    if angle2 < 0:
        angle2 += (1 + int(-angle2) / 360) * 360
