...
through the remote interface of \textbf{\emph{Nebulosity 2}} by Stark Labs, 
\textbf{\emph{Astro Photography Tool}} by Incanus or
...
\subsubsection{Installing AstroTortilla, Cygwin and Astrometry.net}

For ristalling the Linux based Astrometry.net engine locally on a Windows 
platform, the Cygwin emulator environment is needed. The installation is easy
since the Cygwin and Astrometry.net installers are included in the 
AstroTortilla installer executable.
...
the rest indices taking up about 24 GB in total. The smallest field solvable
with Astrometry.net is about five arc minutes.

\textbf{1. Getting the installer}

You’ll find the installer at
\surl{http://astrotortilla.sourceforge.net/}. The green download icon generated
by SourceForge unfortunately not always points to the newest version, so it'd be wise to check out the Downloads page to ensure you get the newest version.
Download and run
\\\texttt{AstroTortilla-<version>.exe}. After running, just repeatedly click
Next until the installer asks you whether to install only AstroTortilla or the 
whole bunch with Cygwin and Astrometry.net.

When installing AstroTortilla for the first time, choose to install Cygwin and
Astrometry.net, too. When updating AstroTortilla, you don't have to install 
Cygwin provided it's still on your hard disk.

The installer will now launch another installer for Cygwin. Here too you can 
keep clicking Next until the installer asks you to "Choose A Download Site".
...
\surl{http://downloads.sourceforge.net} site is preselected. Select one
...
Before continuing, \textbf{make sure you have two sites selected!!!} Believe 
it or not, most error reports we get result from failing to do this step 
correctly. 
...
described in \surl{http://trac.astrometry.net/browser/trunk/src/astrometry/GETTING-INDICES}.  

Note that for now redistributing the
...
We are currently working on generating our own set of indices that we will be
able to redistribute at our own terms (completely freely). Until then, you'll
have to get your indices via Astrometry.net and respect their licensing terms.

\textbf{4. Extracting indices}

After downloading the indices you want, you need to extract the contents of 
the bz2-files into the right directory. If you know how, you can extract them
with the compression software of your liking (WinRAR, 7-zip etc.), provided 
you make sure that after the operation the directory pointed below contains 
FITS-files.

If you don't know how, just follow the instructions below, precisely.

Place the indices you downloaded in
