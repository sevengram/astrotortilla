...
Select a way to connect to your camera. There's native support for \textbf{Astro Photography Tool}, \textbf{MaxIm DL} and \textbf{Nebulosity 2/3}. With MaxIm DL and Nebulosity you can set the imaging filter and binning used for exposures. With Nebulosity you should also set the image directory reset, as we currently cannot read the value from Nebulosity. AstroTortilla provides two macros for giving the current date, \textit{\%(date)s} for current date and \textit{\%(night)s} for rolling the date at noon. Both are formatted as YYYY-MM-DD. There is typically no need to select the actual camera for MaxIm DL or Nebulosity.
